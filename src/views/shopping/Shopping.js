...
const Compras = () => {
  ...
  const [gustosCliente, setGustosCliente] = useState("");
  ...
  const fetchingIngredientes = async () => {
    setIsLoadingChef(true);
    try {
      const response = await fetch(
        `${API_URL}/cliente/ingredientes/${sesion.info.id}`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
      ...
      if (response.ok) {
        const json = await response.json();
        setGustosCliente(json.body.data);
      } else {
        const json = await response.json();
        console.error(json);
      }
  ...
  const sugerencias = useMemo(() => {
    ...
    //Array de gusto
    const gustosArray = gustosCliente.toLowerCase().split(",");
    //Filtrado
    return platos.data.filter((plato) => {
      const ingredientesPlato = plato.ingredientes.toLowerCase().split(",");

      return gustosArray.some((gusto) =>
        ingredientesPlato.includes(gusto.trim())
      );
    });
  }, [platos, gustosCliente]);
  //Catalogo
                ...
                <Row>
                  {isSesion ? (
                    <Col lg="12" className="text-center">
                      <h2>Sugerencias en base a tus gustos</h2>

                      {sugerencias.length === 0 ? (
                        <Alert className="alert-default">
                          <strong>Cero</strong> coincidencias{" "}
                        </Alert>
                      ) : (
